...
export const patientRegister = TryCatch(
  async (
...
export const login = TryCatch(async (req, res, next) => {
  ...
  if (!authenticate) return next(new ErrorHandler("Invalid Password", 400));
...
export const addNewAdmin = TryCatch(
  async (
    req: Request<{}, {}, NewUserRequestBody>,
    res: Response,
    next: NextFunction
...
export const addNewDoctor = TryCatch(async (req, res, next) => {
  ...
  if (!docAvatar) return next(new ErrorHandler("Please Add Photo", 400));

  // 2. Check if all required fields are provided including gender
  if (
    !firstName ||
    !lastName ||
    !email ||
  ...
  // 3. Create the new user (doctor) including the gender field
